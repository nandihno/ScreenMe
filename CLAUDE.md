...
- **Permissions**: sandboxed with `com.apple.security.assets.pictures.read-write` ([ScreenMe.entitlements](ScreenMe/ScreenMe.entitlements)). Screen Recording is granted to the system `screencapture` tool, not to ScreenMe directly. The "Screen Recording Settings" button is shown on `.failed` as a fallback.
